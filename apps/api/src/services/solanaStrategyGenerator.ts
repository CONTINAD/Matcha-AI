...
const SOLANA_STRATEGY_TEMPLATES: SolanaStrategyTemplate[] = [
  ...
  {
    name: 'Solana MACD Trend Following',
    description: 'Rides established trends on Solana with MACD signals. Optimized for trending markets and small accounts.',
    timeframe: '15m',
    universe: ['SOL', 'USDC', 'BONK'],
    indicators: {
      rsi: { period: 14, overbought: 70, oversold: 30 },
      ema: { fast: 12, slow: 26 },
      macd: { fast: 12, slow: 26, signal: 9 }, // MACD focus
      bollinger: { period: 20, stdDev: 2 },
      atr: { period: 14 },
    },
    riskLimits: {
    ...
    solanaOptimizations: {
      useJupiterRouting: true,
      prioritizeSpeed: false, // Can be slower for trends
      minLiquidity: 10000,
      maxSlippage: 1.0,
    },
  },
  {
    name: 'Solana Small Account Optimized',
    description: 'Optimized for $20 accounts. Fast execution, low fees, quick profits. Perfect for learning and small capital.',
    timeframe: '5m',
    universe: ['SOL', 'USDC'],
    indicators: {
      rsi: { period: 14, overbought: 70, oversold: 30 },
    ...
    riskLimits: {
      maxPositionPct: 10, // Conservative for small account
      maxDailyLossPct: 2, // Protect capital
      maxLeverage: 1,
      stopLossPct: 1.5, // Tight stops
    ...
    thresholds: {
      minVolume: 1.0,
      minConfidence: 0.6,
      minLiquidity: 5000, // Lower for small account
    },
    entryRules: {
      momentum: true,
      meanReversion: true,
    ...
    solanaOptimizations: {
      ...
      prioritizeSpeed: true,
      minLiquidity: 5000,
      maxSlippage: 0.5, // Tight slippage for small account
    },
  },
];
